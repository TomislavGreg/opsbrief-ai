...
| Endpoint | Returns |
|----------|---------|
| `GET /events` | Stored events, newest first, filtered by source, type, severity or status, narrowed to a single entity with `entity_type` and `entity_id` or to an occurrence-time window with `occurred_from` and `occurred_to`, and paginated. |
| `GET /events/{event_id}` | One stored event, or 404. |
| `GET /risks` | The current risks across all stored events, most urgent first, each naming the rule and source events behind it, optionally filtered by `severity` or `rule`. |
...
`POST /incidents`, `POST /incidents/{incident_id}/transition`,
`POST /incidents/{incident_id}/resolution`, `POST /incidents/{incident_id}/events`
and `DELETE /incidents/{incident_id}/events/{event_id}`, when it wants OpsBrief AI
to track a disruption as a stateful record, drive it through its lifecycle and edit
the events cited behind it as the picture develops. This is optional: the read path
alone gives the platform briefs and risks without declaring anything.
