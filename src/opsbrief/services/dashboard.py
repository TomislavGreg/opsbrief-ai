...
gathers what the page shows: the service identity, a bounded newest-first view of
the most recent stored events, and the fixed set of links into the JSON endpoints
the later dashboard views render inline. The identity comes from the settings and
the recent-events panel from the event store, read the same way the ``GET /events``
listing reads it.
"""
...
from opsbrief.web import DashboardLink, DashboardView, RecentEventRow
...
def build_dashboard_view(
    ...
    *,
    ...
    The identity comes from the settings and the package version; the recent-events
    panel is the ``recent_limit`` most recently occurred events, newest first, read
    from ``store`` the same way the ``GET /events`` listing reads it, alongside the
    total number of stored events so the panel can say when it is showing a bounded
    view. On an empty store the panel is empty and the rest of the page is unchanged.
    """
    if recent_limit < 1:
