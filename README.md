...
- Reliable timestamp and database-path handling: stored timestamps use a
  fixed-width UTC text with a four-digit year, so any supported instant (years 1
  through 9999, with fractional seconds and any input offset) round-trips through
  the API, SQLite and back for events and incidents and sorts in string order,
  rather than a year below 1000 serialising to a value the reader could not parse.
  A database URL's path is expanded once, so a leading `~` resolves to the home
  directory, a relative path stays relative to the working directory, and an
  absolute path is used as given, with the database never created at a different
  location than the directory prepared for it.
...
- A `POST /webhooks/events` endpoint: an authenticated front door over batch
  ...
  unauthenticated write. The blocking work behind a delivery (signature check,
  parsing and the SQLite write) runs in a worker thread rather than on the event
  loop, so a slow persistence step cannot stall the other requests the service is
  serving; the response is still returned only after the transaction commits.
...
- Input validation aligned with what storage and generated output can hold: a
  whitespace-only entity type, entity id or external id is refused rather than
  normalised to an empty string, a non-finite metadata number (NaN or infinity,
  which JSON cannot represent) is rejected instead of stored and serialised as
  null, and an incident title that is only whitespace is refused. An incident's
  cited event ids are bounded in count and each capped at the same length a stored
  event id and a source reference allow, so a cited id can always resolve into a
  reference rather than failing when a summary or timeline is built. A new incident
  declaration or event link must cite ids the event store actually holds, refused
  with 422 otherwise, while reading a stored incident still names a cited event the
  store no longer holds as a gap rather than failing.
...
- A worked match-operations daily brief example: `build_sample_match_brief` runs
  the whole risk-to-brief pipeline over the match-day fixture at a fixed match-day
  instant, surfacing the overdue pitch inspection, the blocked scoreboard
...
- Safe-by-default write exposure: a read-only mode (`OPSBRIEF_READ_ONLY`, on by
  default whenever demo-data mode is) refuses every write route (event and batch
  ingestion, incident declaration and mutation, and the webhook) with a consistent
  403 ahead of routing while the read endpoints keep working, so a public demo or a
  read-only replica serves reads without taking writes over HTTP; startup seeding
  runs in-process and is unaffected. The bundled Compose file publishes the API on
  the loopback interface only, so a fresh run is not reachable from the network
  without a deliberate change, and `docs/deployment.md` gives a tested-shape ingress
  example that exposes the signed webhook while blocking the other write routes at
  the proxy, noting that the webhook HMAC authenticates only its own route.
- Environment-backed configuration via `OPSBRIEF_`-prefixed variables.
- Configuration validated at startup, so a misconfiguration fails loudly rather
  than at the first request. The AI provider name, the excluded AI context fields,
  the database URL and the logging level are all checked when the application is
  built (and by the `opsbrief` CLI before it runs), with no network call and no
  database opened, so a valid offline configuration still starts; an unknown value
  raises an actionable error naming the setting at fault. The stores are opened
  inside one cleanup scope and the application state is published only once both are
  open and any demo seeding has succeeded, so a failure part-way through closes every
  already-open resource rather than leaking one. `OPSBRIEF_LOG_LEVEL` is applied to
  the root logger, and a CLI configuration fault prints one concise line and exits
  non-zero rather than a traceback.
- A readiness probe that stays cheap and says nothing it should not: `GET /health/ready`
  probes each store with a single-row schema-aware query rather than a whole-table
  count, so it costs the same on a large database as an empty one, and reports a
  degraded dependency with a fixed safe category (the schema is unavailable, or the
  store is unavailable) rather than the raw exception text, so a database path or a
  driver message never leaks through a public probe.
...
The match day doubles as a worked example of the whole pipeline.
`load_sample_match_stored_events` turns the fixture into stored events ready for
the risk rules, a brief context or an incident timeline: each event's
`external_id` becomes its stored `id`, so the same fixture always resolves to the
same source event ids and anything built on it is reproducible.
`build_sample_match_brief` uses that to run the full risk-to-brief pipeline at a
fixed match-day instant, phrased by a scripted fake provider, so a complete brief
can be shown without a database, a running server or a real model:

```python
from opsbrief.samples import build_sample_match_brief

brief = build_sample_match_brief()
brief.summary  # an illustrative duty-manager summary, phrased by the fake provider
brief.risks  # the overdue, blocked and repeated-failure risks the rules found
brief.source_event_ids  # every fixture event the brief traces back to
brief.confidence  # high: every cited event resolves and the picture is phrased
```

Only the `summary` is illustrative phrasing; the risks, the source event ids and
the confidence underneath it are the real deterministic picture the service
...
risks most urgent first. Each risk shows its severity as a badge and names the rule
and the source events behind it, carried straight from the rule's deterministic
...
| AI-103 | Validate configuration at startup and make readiness truthful | Correctness and safety | Done |
...
unavailable tool never stalls unrelated eligible work. AI-103 is now Done, closing
the P2 correctness and safety work bar the Docker-gated AI-101, so the Ready queue
is replenished with AI-106 (reuse one reporting context per dashboard request), the
next eligible P2 whose dependencies are all Done, opening the efficiency and
reporting phase. AI-102, AI-104 and AI-105 are also Done, clearing the P1
correctness and persistence bugs.
...
- 2026-09-21 - Validated configuration at startup and made readiness truthful (AI-103): an unknown provider served readiness 200 then failed `/brief` with 500, exclusion settings were parsed lazily, the stores were opened before the lifespan cleanup scope so a failed second open or seeding leaked a connection, `log_level` was unused, and readiness counted the whole table and reported raw exception strings. Added `opsbrief.startup` with `validate_settings`, which checks the provider name, the excluded AI context fields, the database URL and the log level with no network call and no database opened, raising an actionable error naming the setting at fault, and `configure_logging`, which applies the level to the root logger. Wired both into `create_app` and moved store opening into an `ExitStack` that publishes application state only once both stores are open and seeding has succeeded, so a part-way failure closes every already-open resource. Gave the CLI a concise configuration-error line and a non-zero exit rather than a traceback. Replaced the readiness count with a cheap single-row schema-aware `ping` on each store and mapped any probe failure to a fixed safe category, so a large database is not scanned and no path or driver message leaks. Added startup, main-lifecycle, store-ping, CLI and readiness tests.
...
- 2026-09-16 - Made external exposure and public demo writes safe by default (AI-102): with a webhook secret set an unsigned `POST /events` still returned 201, the incident mutations were unauthenticated, Compose published the port on every interface, and demo mode still accepted writes. Added a read-only mode (`OPSBRIEF_READ_ONLY`, on by default whenever demo-data mode is) enforced by a `ReadOnlyMiddleware` that refuses every unsafe-method request (`POST`, `PUT`, `PATCH`, `DELETE`) with a consistent 403 ahead of routing, so event and batch ingestion, incident declaration and mutation, and the webhook are all closed at once while `GET` reads keep working; startup seeding runs in-process and is unaffected. Changed the bundled `compose.yaml` to publish on the loopback interface only, so a fresh run is not reachable from the network without a deliberate change, and documented a tested-shape nginx ingress in `docs/deployment.md` that exposes the signed webhook and blocks the other write routes at the proxy, noting the webhook HMAC authenticates only its own route. Added middleware unit tests, per-route write-refusal coverage across the mutation surface, and demo-mode read-but-not-write tests.
